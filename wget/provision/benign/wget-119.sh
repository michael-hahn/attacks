...
sleep 6
/usr/local/bin/wget --tries=2 -N gov.uk/
sleep 4
...
sleep 5
/usr/local/bin/wget --tries=2 -nv yandex.ru/
sleep 4
/usr/local/bin/wget http://mirrors.kernel.org/ubuntu/pool/universe/s/sx/sx_2.0+ds-4build2_amd64.deb
sudo dpkg -i sx_2.0+ds-4build2_amd64.deb
rm sx_2.0+ds-4build2_amd64.deb
...
sleep 6
/usr/local/bin/wget --tries=2 --content-on-error cnn.com/
sleep 5
...
rm sx_2.0+ds-4build2_amd64.deb
sleep 8
/usr/local/bin/wget --tries=2 -o /dev/null pinterest.com/
sleep 6
...
rm dc_1.07.1-2_amd64.deb
sleep 6
/usr/local/bin/wget --tries=2 --no-cookies tinyurl.com/
sleep 5
...
sleep 7
/usr/local/bin/wget --tries=2 --save-headers typepad.com/
sleep 3
...
sleep 10
cp /tmp/audit.log /vagrant/data/wget-normal-119.log
cd ..
rm -rf wgets
