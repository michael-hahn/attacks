...
sleep 5
/usr/local/bin/wget --tries=2 --keep-session-cookies forbes.com/
sleep 5
...
sleep 3
/usr/local/bin/wget --tries=2 --ignore-length youtube.com/
sleep 5
/usr/local/bin/wget --tries=2 --no-cookies w3.org/
sleep 5
...
rm dc_1.07.1-2_amd64.deb
sleep 7
/usr/local/bin/wget --tries=2 --follow-ftp ovh.net/
sleep 3
...
sleep 5
/usr/local/bin/wget --tries=2   tumblr.com/
sleep 7
...
sleep 2
/usr/local/bin/wget --tries=2 --auth-no-challenge taobao.com/
sleep 6
/usr/local/bin/wget http://ftp.osuosl.org/pub/ubuntu/pool/universe/f/fonts-adf/fonts-adf-accanthis_0.20110505-1_all.deb
sudo dpkg -i fonts-adf-accanthis_0.20110505-1_all.deb
rm fonts-adf-accanthis_0.20110505-1_all.deb
...
rm fonts-adf-accanthis_0.20110505-1_all.deb
sleep 8
/usr/local/bin/wget --tries=2 --no-check-certificate twitter.com/
sleep 5
/usr/local/bin/wget http://mirrors.kernel.org/ubuntu/pool/universe/s/sx/sx_2.0+ds-4build2_amd64.deb
sudo dpkg -i sx_2.0+ds-4build2_amd64.deb
rm sx_2.0+ds-4build2_amd64.deb
...
sleep 10
cp /tmp/audit.log /vagrant/data/wget-normal-114.log
cd ..
rm -rf wgets
