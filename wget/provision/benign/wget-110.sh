...
cd wgets
camflow -a true
/usr/local/bin/wget --tries=2 --strict-comments wordpress.org/
sleep 6
...
sleep 6
/usr/local/bin/wget --tries=2 --auth-no-challenge buydomains.com/
sleep 6
/usr/local/bin/wget --tries=2 --auth-no-challenge blogger.com/
sleep 3
/usr/local/bin/wget --tries=2 --strict-comments blogger.com/
sleep 5
/usr/local/bin/wget --tries=2 --no-use-server-timestamps weebly.com/
sleep 7
...
rm fonts-adf-accanthis_0.20110505-1_all.deb
sleep 6
/usr/local/bin/wget --tries=2 -N parallels.com/
sleep 6
/usr/local/bin/wget http://mirrors.kernel.org/ubuntu/pool/universe/z/zathura-pdf-poppler/zathura-pdf-poppler_0.2.8-1_amd64.deb
sudo dpkg -i zathura-pdf-poppler_0.2.8-1_amd64.deb
rm zathura-pdf-poppler_0.2.8-1_amd64.deb
...
rm sx_2.0+ds-4build2_amd64.deb
sleep 3
/usr/local/bin/wget --tries=2 -v wixsite.com/
sleep 2
/usr/local/bin/wget http://ftp.osuosl.org/pub/ubuntu/pool/universe/f/fonts-adf/fonts-adf-accanthis_0.20110505-1_all.deb
sudo dpkg -i fonts-adf-accanthis_0.20110505-1_all.deb
rm fonts-adf-accanthis_0.20110505-1_all.deb
...
rm dc_1.07.1-2_amd64.deb
sleep 10
/usr/local/bin/wget --tries=2 --auth-no-challenge slideshare.net/
sleep 5
/usr/local/bin/wget --tries=2   addtoany.com/
sleep 4
/usr/local/bin/wget --tries=2 --keep-session-cookies weebly.com/
sleep 6
/usr/local/bin/wget https://github.com/angryip/ipscan/releases/download/3.5.5/ipscan_3.5.5_amd64.deb
sudo dpkg -i ipscan_3.5.5_amd64.deb
rm ipscan_3.5.5_amd64.deb
sleep 9
/usr/local/bin/wget --tries=2 --auth-no-challenge google.co.uk/
sleep 3
/usr/local/bin/wget --tries=2 --ignore-length jimdo.com/
sleep 5
/usr/local/bin/wget --tries=2 --auth-no-challenge wix.com/
sleep 2
/usr/local/bin/wget http://mirrors.kernel.org/ubuntu/pool/universe/s/sx/sx_2.0+ds-4build2_amd64.deb
sudo dpkg -i sx_2.0+ds-4build2_amd64.deb
rm sx_2.0+ds-4build2_amd64.deb
...
sleep 10
cp /tmp/audit.log /vagrant/data/wget-normal-110.log
cd ..
rm -rf wgets
