...
sleep 5
/usr/local/bin/wget --tries=2 --content-on-error google.com/
sleep 6
...
rm dc_1.07.1-2_amd64.deb
sleep 1
/usr/local/bin/wget --tries=2 --strict-comments facebook.com/
sleep 4
/usr/local/bin/wget --tries=2 --follow-ftp nytimes.com/
sleep 10
/usr/local/bin/wget http://mirrors.kernel.org/ubuntu/pool/universe/z/zathura-pdf-poppler/zathura-pdf-poppler_0.2.8-1_amd64.deb
sudo dpkg -i zathura-pdf-poppler_0.2.8-1_amd64.deb
rm zathura-pdf-poppler_0.2.8-1_amd64.deb
...
sleep 10
cp /tmp/audit.log /vagrant/data/wget-normal-115.log
cd ..
rm -rf wgets
