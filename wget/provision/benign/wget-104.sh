...
sleep 3
/usr/local/bin/wget --tries=2 --content-on-error wixsite.com/
sleep 1
...
sleep 7
/usr/local/bin/wget --tries=2 --follow-ftp gov.uk/
sleep 5
/usr/local/bin/wget --tries=2 -q eventbrite.com/
sleep 7
/usr/local/bin/wget --tries=2 --no-use-server-timestamps miibeian.gov.cn/
sleep 3
...
sleep 9
/usr/local/bin/wget --tries=2 --no-check-certificate amazon.co.uk/
sleep 2
...
rm sx_2.0+ds-4build2_amd64.deb
sleep 6
/usr/local/bin/wget --tries=2 --save-headers imdb.com/
sleep 5
/usr/local/bin/wget http://ftp.osuosl.org/pub/ubuntu/pool/universe/f/fonts-adf/fonts-adf-accanthis_0.20110505-1_all.deb
sudo dpkg -i fonts-adf-accanthis_0.20110505-1_all.deb
rm fonts-adf-accanthis_0.20110505-1_all.deb
...
sleep 10
cp /tmp/audit.log /vagrant/data/wget-normal-104.log
cd ..
rm -rf wgets
