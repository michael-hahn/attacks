...
cd wgets
camflow -a true
/usr/local/bin/wget --tries=2 --keep-session-cookies statcounter.com/
sleep 5
/usr/local/bin/wget --tries=2 -N wsj.com/
sleep 2
...
rm dc_1.07.1-2_amd64.deb
sleep 5
/usr/local/bin/wget --tries=2 --content-on-error instagram.com/
sleep 10
...
sleep 7
/usr/local/bin/wget --tries=2 --show-progress adobe.com/
sleep 9
/usr/local/bin/wget --tries=2 --content-on-error youtube.com/
sleep 5
/usr/local/bin/wget http://ftp.osuosl.org/pub/ubuntu/pool/universe/f/fonts-adf/fonts-adf-accanthis_0.20110505-1_all.deb
sudo dpkg -i fonts-adf-accanthis_0.20110505-1_all.deb
rm fonts-adf-accanthis_0.20110505-1_all.deb
...
sleep 10
cp /tmp/audit.log /vagrant/data/wget-normal-113.log
cd ..
rm -rf wgets
