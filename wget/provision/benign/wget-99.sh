...
cd wgets
camflow -a true
/usr/local/bin/wget --tries=2 --no-check-certificate livejournal.com/
sleep 5
...
sleep 3
/usr/local/bin/wget --tries=2 -N wixsite.com/
sleep 5
...
rm zathura-pdf-poppler_0.2.8-1_amd64.deb
sleep 4
/usr/local/bin/wget --tries=2   vk.com/
sleep 3
...
sleep 5
/usr/local/bin/wget --tries=2 --follow-ftp digg.com/
sleep 9
/usr/local/bin/wget http://mirrors.kernel.org/ubuntu/pool/universe/z/zathura-pdf-poppler/zathura-pdf-poppler_0.2.8-1_amd64.deb
sudo dpkg -i zathura-pdf-poppler_0.2.8-1_amd64.deb
rm zathura-pdf-poppler_0.2.8-1_amd64.deb
...
/usr/local/bin/wget --tries=2   vk.com/
sleep 5
/usr/local/bin/wget --tries=2 --auth-no-challenge reddit.com/
sleep 2
/usr/local/bin/wget --tries=2   vk.com/
sleep 6
...
rm fonts-adf-accanthis_0.20110505-1_all.deb
sleep 5
/usr/local/bin/wget --tries=2 --no-use-server-timestamps blogspot.com/
sleep 5
/usr/local/bin/wget http://mirrors.kernel.org/ubuntu/pool/universe/z/zathura-pdf-poppler/zathura-pdf-poppler_0.2.8-1_amd64.deb
sudo dpkg -i zathura-pdf-poppler_0.2.8-1_amd64.deb
rm zathura-pdf-poppler_0.2.8-1_amd64.deb
