...
cd wgets
camflow -a true
/usr/local/bin/wget --tries=2 -q cloudflare.com/
sleep 3
/usr/local/bin/wget --tries=2 --auth-no-challenge linkedin.com/
sleep 2
/usr/local/bin/wget --tries=2 --save-headers dropbox.com/
sleep 7
...
sleep 9
/usr/local/bin/wget --tries=2 -nv huffingtonpost.com/
sleep 6
/usr/local/bin/wget --tries=2 --ignore-length telegraph.co.uk/
sleep 8
/usr/local/bin/wget --tries=2 --no-use-server-timestamps wix.com/
sleep 1
/usr/local/bin/wget --tries=2 --follow-ftp ebay.com/
sleep 4
...
sleep 6
/usr/local/bin/wget --tries=2 --auth-no-challenge theguardian.com/
sleep 6
/usr/local/bin/wget --tries=2 --show-progress slideshare.net/
sleep 7
/usr/local/bin/wget --tries=2   bbc.co.uk/
sleep 4
...
sleep 6
/usr/local/bin/wget --tries=2 --show-progress washingtonpost.com/
sleep 4
...
sleep 6
/usr/local/bin/wget --tries=2 -V buydomains.com/
sleep 5
/usr/local/bin/wget https://github.com/angryip/ipscan/releases/download/3.5.5/ipscan_3.5.5_amd64.deb
sudo dpkg -i ipscan_3.5.5_amd64.deb
rm ipscan_3.5.5_amd64.deb
...
rm fonts-adf-accanthis_0.20110505-1_all.deb
sleep 3
/usr/local/bin/wget --tries=2 --save-headers google.co.jp/
sleep 7
