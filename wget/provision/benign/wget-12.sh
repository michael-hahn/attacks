...
sleep 7
/usr/local/bin/wget --tries=2 -N paypal.com/
sleep 8
...
rm sx_2.0+ds-4build2_amd64.deb
sleep 5
/usr/local/bin/wget --tries=2 -V 123-reg.co.uk/
sleep 4
/usr/local/bin/wget --tries=2 -nv bbc.co.uk/
sleep 7
/usr/local/bin/wget --tries=2 --no-dns-cache amazon.co.uk/
sleep 5
/usr/local/bin/wget http://ftp.osuosl.org/pub/ubuntu/pool/universe/f/fonts-adf/fonts-adf-accanthis_0.20110505-1_all.deb
...
sleep 5
/usr/local/bin/wget --tries=2 -N about.com/
sleep 5
/usr/local/bin/wget --tries=2 --no-use-server-timestamps nih.gov/
sleep 7
/usr/local/bin/wget --tries=2 --show-progress vk.com/
sleep 1
