...
sleep 4
/usr/local/bin/wget --tries=2 --content-on-error imdb.com/
sleep 2
...
sleep 6
/usr/local/bin/wget --tries=2 -v etsy.com/
sleep 2
...
rm sx_2.0+ds-4build2_amd64.deb
sleep 3
/usr/local/bin/wget --tries=2 -N feedburner.com/
sleep 4
/usr/local/bin/wget http://mirrors.kernel.org/ubuntu/pool/universe/s/sx/sx_2.0+ds-4build2_amd64.deb
sudo dpkg -i sx_2.0+ds-4build2_amd64.deb
rm sx_2.0+ds-4build2_amd64.deb
...
rm sx_2.0+ds-4build2_amd64.deb
sleep 5
/usr/local/bin/wget --tries=2 --strict-comments github.com/
sleep 4
/usr/local/bin/wget --tries=2 --save-headers sourceforge.net/
sleep 3
...
sleep 6
/usr/local/bin/wget --tries=2 --show-progress wixsite.com/
sleep 5
/usr/local/bin/wget --tries=2 --no-dns-cache dailymail.co.uk/
sleep 3
/usr/local/bin/wget --tries=2 -q godaddy.com/
sleep 4
/usr/local/bin/wget --tries=2   bloomberg.com/
sleep 5
sudo camflow -a false
sleep 10
cp /tmp/audit.log /vagrant/data/wget-normal-100.log
cd ..
rm -rf wgets
